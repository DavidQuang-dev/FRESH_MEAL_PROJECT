    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (cart != null && user != null) {
                int rs = odao.insertOrder(Integer.parseInt(addressID), Integer.parseInt(deliveryID), Integer.parseInt(discount), Integer.parseInt(tax), Integer.parseInt(totalPrice), paymentMethod, user.getUserID(), cart);
                if (rs >= 1) {
                    Email email = new Email();
                    email.sendEmail(email.subjectNewOrder(), "Cảm ơn đã đặt hàng", user.getEmail());
